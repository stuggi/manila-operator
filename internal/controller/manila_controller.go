...
// +kubebuilder:rbac:groups="security.openshift.io",resourceNames=hostmount-anyuid;nonroot-v2,resources=securitycontextconstraints,verbs=use
...
func (r *ManilaReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, _err error) {
	...
	// Initialize notification bus condition if configured
...
func (r *ManilaReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager) error {
	...
	if err := mgr.GetFieldIndexer().IndexField(context.Background(), &manilav1beta1.Manila{}, authAppCredSecretField, func(rawObj client.Object) []string {
		// Extract the application credential secret name from the spec, if one is provided
		cr := rawObj.(*manilav1beta1.Manila)
	...
	rbacRules := []rbacv1.PolicyRule{
		{
			...
			ResourceNames: []string{"hostmount-anyuid", "nonroot-v2"},
...
// transportURLCreateOrUpdate - this function is called in two different scenarios:
// 1. request a messagingBus: it returns the regular transportURL that is used to
// extract the messagingBus URL string
// 2. request a notificationBus: it returns a transportURL that is used to extract
// the notificationBus URL string
func (r *ManilaReconciler) transportURLCreateOrUpdate(
	...
	var rmqName string
	if isNotifications {
		rmqName = fmt.Sprintf("%s-manila-notifications-transport", instance.Name)
	} else {
		rmqName = fmt.Sprintf("%s-manila-transport", instance.Name)
	}
