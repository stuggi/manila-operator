...
test: manifests generate fmt vet envtest ginkgo ## Run tests.
	...
	$(GINKGO) --trace --cover --coverpkg=../../internal/...,../../api/v1beta1/... --coverprofile cover.out --covermode=atomic --randomize-all ${PROC_CMD} $(GINKGO_ARGS) ./test/functional/...
.PHONY: gowork
...
operator-lint: $(LOCALBIN) gowork
	...
	cd ./api && go vet -vettool=$(LOCALBIN)/operator-lint ./...
...
force-bump: ## Force bump operator and lib-common dependencies
	for dep in $$(cat go.mod | grep openstack-k8s-operators | grep -vE -- 'indirect|manila-operator|^replace' | awk '{print $$1}'); do \
		go get $$dep@$(BRANCH) ; \
	done
	for dep in $$(cat api/go.mod | grep openstack-k8s-operators | grep -vE -- 'indirect|manila-operator|^replace' | awk '{print $$1}'); do \
